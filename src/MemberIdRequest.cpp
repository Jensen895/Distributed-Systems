...
MemberIdRequest::MemberIdRequest(MESSAGE_TYPE message_type, string memberId, string membershiplist)
    : message_type(message_type), memberId(memberId), membershiplist(membershiplist)
{
}

const string MemberIdRequest::serialize() { return to_string(message_type) + CUSTOM_DELIMITER + memberId + CUSTOM_DELIMITER + membershiplist; }

MemberIdRequest MemberIdRequest::deserialize(const string &serializedData)
{
    ...
    if (delimiterPos != string::npos && delimiterPos < serializedData.length() - 1)
    {
        ...
        if (delimiterPos2 != std::string::npos)
        {
            ...
            return MemberIdRequest(message_type, memberId, membershiplist);
        }
        else
        {
